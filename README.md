...
> [!重要]
> **此项目不是 Spring AI / Spring AI Alibaba 的套壳项目。** 框架提供的是模型调用、向量存储这些基础能力。
> 但三层执行器路由、前置编排器五步决策链、双通道检索融合、Parent-Child 块聚合、证据预算控制与无证据短路、组合式切块引擎、摘要压缩记忆、集群级租约互斥、全链路观测追踪——这些构成项目核心竞争力的能力，全部是在框架之上从零设计和手写实现的。光靠框架远远撑不起一个企业级 AI 系统，真正的深度在框架之外。
